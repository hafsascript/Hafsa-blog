import React from 'react';
import { Link } from 'react-router-dom';

export default function PostCard({post}) {
  return (
    <div className='group relative w-full border h-[350px] overflow-hidden rounded-lg sm:w-[350px]
    border-orange-400 hover:border-2
    transition-all'>
        <Link to={`/post/${post.slug}`}>
            <img className='h-[260px] w-full
            object-cover group-hover:h-[200px]
            transition-all duration-300 z-20'src={post.image}/>
        </Link>
        <div className='p-3 flex flex-col gap-2'>
            <p className='text-lg font-semibold line-clamp-2'>{post.title}</p>
            <p className='text-xs'>{post.category}</p>
            <Link className='z-10 group-hover:bottom-0
            absolute bottom-[-200px]
            left-0 right-0
            border border-green-400 text-orange-500
            hover:bg-yellow-100 hover:text-green-400
            transition-all duration-300
            text-center py-2 rounded-sm !rounded-tl-none m-2' to={`/post/${post.slug}`}>
                Read Article
            </Link>
        </div>
    </div>
  )
}
